Shader "Unity Shader Book/Chapter 8/AlphaBlendZWrite"
{
    ...
    SubShader
    {
        ...
        // 添加一个通道，这个通道开启深度写入，该通道会参与深度测试，并会因为被其他片元遮挡导致被丢弃，这正是我们想要的效果，即被遮挡的半透明片元被丢弃
        Pass
        {
            // 开启z深度写入
            ZWrite On
            // 设置颜色写入掩码为0，当颜色写入掩码为0时，这个通道不会向片元着色器内部写入任何颜色
            // 颜色掩码的设置方式为 ColorMask RGB | A | 0 | 其他任何R、G、B、A颜色的组合
            ColorMask 0
        }
